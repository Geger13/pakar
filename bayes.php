...
$user = "";
$dtgs = $_POST['daftargejala'];
$my_array1 = implode(",", $dtgs);
$my_string = $my_array1;
$my_array2 = explode(",", $my_string);
$dataGejala = $my_array2;
...
foreach ($list_penyakit_k as $nil_penyakit_k) {
    ...
    if (mysqli_num_rows($validasi) == 0) {
        $simpan =  mysqli_query($konek, "INSERT INTO tblbantu_2 SET kd_penyakit='$nil_penyakit_k[1]', jml_gejala='1'");
    } else {
...
if (count($list_penyakit_penuh_kriteria) == 0) {
    ...
    foreach ($dataGejala as $nilai) {
        ...
        $r_gejala = mysqli_query($konek, $q_gejala);
    ...
    $r_penyakit = mysqli_query($konek, $q_penyakit);
    while ($data_penyakit = mysqli_fetch_array($r_penyakit)) {
        ...
        foreach ($dataGejala as $cek_gejala) {
            ...
            $r_gejala2 = mysqli_query($konek, $q_gejala2);
    ...
    foreach ($hit_persen as $key) {
        ...
        $r_persen = mysqli_query($konek, $q_persen);
    }

    $q_hasilkonsul = "SELECT tbldiagnosa.id_pengunjung, tblpenyakit.kd_penyakit, tblpenyakit.nm_penyakit, tbldiagnosa.persen FROM tbldiagnosa INNER JOIN tblpenyakit ON tbldiagnosa.kd_penyakit=tblpenyakit.kd_penyakit WHERE tbldiagnosa.id_pengunjung='$user'";
    $r_hasilkonsul = mysqli_query($konek, $q_hasilkonsul);
    ...
    $r_u_gejala = mysqli_query($konek, $q_u_gejala);
    $q_d_gejala = "DELETE FROM tbldiagnosa WHERE id_pengunjung='$user'";
    $r_d_gejala = mysqli_query($konek, $q_d_gejala);
...
echo '{ "hasil": "' . round($hs_max['persen'], 2) . '", "penyakit": "' . $hs_max['nm_penyakit'] . '", "penanganan": "' . $hs_max['pengobatan'] . '" }';
?>